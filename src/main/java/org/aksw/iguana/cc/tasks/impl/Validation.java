...
public class Validation implements Task {
    ...
    public Validation(String suiteID, long taskId, Validation.Config config, ResponseBodyProcessorInstances responseBodyProcessorInstances, List<Storage> storages, List<Metric> metrics) {
        ...
        // check if system names are unique
        final var allNames = new ArrayList<String>();
        for (TriplestoreTuple tuple : config.validate) {
            if (allNames.contains(tuple.triplestoreName)) {
                throw new IllegalArgumentException("Triplestore names in validation task must be unique. Duplicate name: " + tuple.triplestoreName);
            }
            allNames.add(tuple.triplestoreName);
        }
        if (allNames.contains(config.groundTruth.triplestoreName)) {
            throw new IllegalArgumentException("Triplestore names in validation task must be unique. Duplicate name: " + config.groundTruth.triplestoreName);
        }
    ...
    public void run() {
        ...
        for (TriplestoreTuple validateTuple : config.validate) {
            ...
            if (mismatchedValues == 0 && missingValues == 0) {
                logger.info("Validation successful for triplestore \"{}\".", validateTuple.triplestoreName);
            } else {
                logger.info("Validation completed for triplestore \"{}\" with {} mismatched values and {} missing values.",
                        validateTuple.triplestoreName, mismatchedValues, missingValues);
            }
        }
    ...
    private Table getTriplestoreResults(StresstestResultReference ref) {
        if (ref instanceof CSVResultPath csvPath) {
            return getTriplestoreResultsFromCSV(csvPath.csvResultPath());
        } else if (ref instanceof TaskID taskId) {
            if (this.csvStorage == null) {
                throw new IllegalStateException("No CSVStorage available to retrieve stresstest results. Result validation via TaskID is not possible.");
            }
            final var path = this.csvStorage.getTaskDirectory(String.valueOf(taskId.stresstestId));
            return getTriplestoreResultsFromCSV(path.toString());
        } else {
            throw new IllegalArgumentException("Unknown StresstestResultReference type.");
        }
    }

    private Table getTriplestoreResultsFromCSV(String csvResultPath) {
        final var path = Paths.get(csvResultPath);

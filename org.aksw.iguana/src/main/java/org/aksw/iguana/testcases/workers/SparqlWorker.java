...
public class SparqlWorker extends Worker implements Runnable {
	...
	public static void main(String[] argc) throws SQLException {
		...
		Connection con = ConnectionFactory.createImplConnection(
				"dbpedia.org/sparql", null, -1);
		...
		while ((Calendar.getInstance().getTimeInMillis() - start
				.getTimeInMillis()) < 20000) {
		}
	...
	public void init(int workerNr) {
		...
		for (int i = 0; i < latencyAmount.size(); i++) {
			...
			latencyAmount
					.set(i, getIntervallLatency(intervall, latStrat, rand));
		}
	...
	protected void waitTime() {
		...
		int latency = 0;
		for (int i = 0; i < latencyAmount.size(); i++) {
			latency += getLatency(latencyAmount.get(i), latencyStrategy.get(i),
					rand);
		}
		log.finest("Waiting " + latency + "ms before next SPARQL Query");
		Calendar start = Calendar.getInstance();
		while ((Calendar.getInstance().getTimeInMillis() - start
				.getTimeInMillis()) < latency) {
			try {
				Thread.sleep(10);

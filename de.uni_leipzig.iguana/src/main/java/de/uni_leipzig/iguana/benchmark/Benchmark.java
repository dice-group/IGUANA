...
//	private static String[] randFiles;
...
//	 *             Signals that an URISyntaxException has occured.
//	 */
...
//							+ Benchmark.class.getName() + " configfile.xml");
//			end = true;
//			return;
//		} else {
...
//
//	/**
//	 * If Benchmark finished unexpected, this sends (if email is enabled) an
//	 * Email with the current Results or write the Excpetion in log file if the
//	 * email couldn't be sended.
//	 */
...
//				if (attach) {
//					try {
//						attachment = ZipUtils.folderToZip("." + File.separator
//								+ Benchmark.TEMP_RESULT_FILE_NAME, "."
//								+ File.separator
//								+ Benchmark.TEMP_RESULT_FILE_NAME + ".zip");
//
...
//	}
	...
	public static void pre() {
		...
		if(Config.logClusterClass!=null){
			ClusterProcessor.clustering(Config.logClusterClass, 
	...
	public static void post() {
		...
		EmailProcessor.send(Config.attach, ResultProcessor.RESULT_FOLDER, startTime, endTime);
	}
...
//	static void start(String pathToXMLFile) {
//
...
//			rootNode = cp.getElementAt("iguana", 0);
...
//			refCon = ConnectionFactory.createConnection(dbNode,
//					config.get("ref-con"));
//			log.info(refCon.getEndpoint());
//			if (email != null) {
//				log.info("Initialize Email...");
//				if (email.get("pwd") == null) {
//					log.info("Password for email-account " + email.get("user")
//							+ " required: ");
//					Console cons;
//					char[] passwd;
...
//
//				}
//				EmailHandler.initEmail(String.valueOf(email.get("hostname")),
...
//				RDFVocabulary rdfV = (RDFVocabulary) Class.forName(
//						config.get("rdf-vocabulary-class")).newInstance();
...
//				// COnverting the Data
//				Converter
//						.rawToFormat(config.get("converter-class"),
...
//				String output = config.get("output-path")
//						+ File.separator
//						+ StringHandler.stringToAlphanumeric(UUID.randomUUID()
//								.toString())
//						+ "."
//						+ FileExtensionToRDFContentTypeMapper
//								.guessFileExtensionFromFormat(config
//										.get("output-format"));
...
//					Config.DBTestType.valueOf(config.get("dbs")),
//					config.get("ref-con"), log);
//
//			// mkdirs
...
//				clustering(config.get("log-cluster"), config.get("log-path"),
//						config.get("log-queries-file"));
...
//				if (config.get(key) == null || config.get(key).isEmpty()) {
//					continue;
//				}
...
//	 * 
//	 * @param name
//	 *            The name of the clustering class to use
//	 * @param logPath
//	 *            The path of the log files to cluster
//	 * @param queriesFile
//	 *            the file name in which the query patterns should be saved in
//	 */
//	private static void clustering(String name, String logPath,
//			String queriesFile) {
//		try {
...
//
//	/**
//	 * Main loop. Tests for every percentage and for every given database the
//	 * given testcases
//	 * 
//	 * @param ids
//	 *            the ids of the databases which should be tested given in the
//	 *            config.xml
//	 * @param pathToXMLFile
//	 *            the name to the config file
//	 * @throws ClassNotFoundException
//	 *             Signals that a ClassNotFoundException has occured.
//	 * @throws SAXException
//	 *             Signals that a SAXEception has occured.
//	 * @throws IOException
//	 *             Signals that an IOException has occurred.
//	 * @throws ParserConfigurationException
//	 *             Signals that a ParserConfigurationException has occured.
//	 * @throws SQLException
//	 *             Signals that a SQLException has occured.
//	 * @throws InterruptedException
//	 *             Signals that an InterruptedException has occured.
//	 */
...
//					if (testcaseWithoutID
//							.equals(UploadTestcase.class.getName())) {
//						isUpload = true;
//						break;
//					}
//				}
//				if (isUpload) {
//					log.info("Upload Testcase starting for " + db + ":"
//							+ percents.get(i));
...
//					log.info("Upload Testcase finished for " + db + ":"
//							+ percents.get(i));
//					upload.save();
//				}
//				// try{
//				// if(config.get("warmup-query-file")!=null
//				// &&config.get("warmup-time")!=null){
...
//				}
//				new File("." + File.separator + RESULT_FILE_NAME
//						+ File.separator + testCase + File.separator + suffix)
//						.mkdirs();
//				res.setFileName("." + File.separator + RESULT_FILE_NAME
//						+ File.separator + testCase + File.separator + suffix
...
//
//	/**
//	 * Warmups the given Connection with the given queries for time in
//	 * miliseconds
//	 * 
//	 * @param con
//	 *            Connection to warump
//	 * @param queries
//	 *            the queries to use for the warmup
//	 * @param time
//	 *            time how long the warump should be executed
//	 */
//	private static void warmup(Connection con, Collection<String> queries,
//			File path, String graphURI, Long time) {
...
//	 *            Connection to warump
//	 * @param queriesFile
//	 *            the queries file in which the queries are saved to use for the
//	 *            warmup
//	 * @param time
//	 *            time how long the warump should be executed
//	 */
//	private static void warmup(Connection con, String queriesFile, String path,
//			String graphURI, Long time) {
...
//
//	/**
//	 * Starts every testcase except for the UploadTestcase for the given
//	 * connection and percentage
//	 * 
//	 * @param con
//	 *            The Connection to test
//	 * @param dbName
//	 *            the name of the connection (the id of the connection in the
//	 *            config file)
//	 * @param percent
//	 *            the percentage on which will be tested
//	 * @throws IOException
//	 *             Signals that an I/O exception has occurred.
//	 */
//	@SuppressWarnings("unchecked")
//	private static void start(Connection con, String dbName, String percent)
//			throws IOException {
//
...
//			try {
//				if (config.get("warmup-query-file") != null
//						&& (config.get("warmup-time") != null && Integer
//								.valueOf(config.get("warmup-time")) != 0)) {
//					log.info("Warmup started");
//					warmup(con,
//							String.valueOf(config.get("warmup-query-file")),
//							config.get("warmup-updates"),
//							config.get("graph-uri"),
//							Long.valueOf(config.get("warmup-time")));
//					log.info("Warmup finished! Ready to get pumped!");
...
//				continue;
//			}
//			log.info("Starting " + testcaseWithoutID + " id: "
//					+ testcase.substring(testcase.lastIndexOf("&") + 1)
//					+ " for " + dbName + ":" + percent);
//			try {
//				Properties testProps = testcases.get(testcase);
//				if (!testProps.containsKey("graphURI")) {
//					if (config.get("graph-uri") != null)
//						testProps.setProperty("graphURI",
//								config.get("graph-uri"));
//				}
//				if (!testProps.containsKey("file")) {
//					if (randFiles != null) {
//						testProps.setProperty("file", randFiles[percents
//								.indexOf(Double.valueOf(percent))]);
//					} else if (config.containsKey("random-hundred-file")) {
//						testProps.setProperty("file",
//								config.get("random-hundred-file"));
//					}
//				}
...
//				percent = percentBack;
//				log.info("Finished " + testcase + " for " + dbName + ":"
//						+ percent);
//
...
//	 * 
//	 * @param con
//	 *            Connection to use if the initial File must be generated
//	 * @param hundredFile
//	 *            the file with 100% data in it
...
//				DataGenerator.generateData(con, config.get("graph-uri"),
//						fileName, outputFile, config.get("random-function"),
//						percents.get(i),
//						Double.valueOf(config.get("coherence-roh")),
//						Double.valueOf(config.get("coherence-ch")));
//			} else {
//				ExtendedDatasetGenerator.generatedExtDataset(fileName,
//						outputFile, percents.get(i));
//			}
